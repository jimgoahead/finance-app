...
@st.cache_data(ttl=3600) # 💡 ปรับความจำชั่วคราวเป็น 1 ชั่วโมง (3600 วินาที) ตามที่เจ้านายสั่งค่ะ
def load_data(): # 👈 เจนนี่เปลี่ยนกลับมาใช้ชื่อเดิม เพื่อแก้ Error ค่ะ
...
# ฟังก์ชันสำหรับปุ่ม Refresh (ล้างความจำเพื่อดึงข้อมูลใหม่)
def refresh_data():
    st.cache_data.clear()
    st.rerun() # เหลือตัวเดียวพอค่ะเจ้านาย ขอบคุณที่ช่วยเช็กนะคะ!

# ดึงข้อมูลมาใช้งาน (เรียกชื่อเดิมได้เลย ไม่ Error แน่นอนค่ะ)
df = load_data()
