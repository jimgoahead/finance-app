...
# ==========================================
# ส่วนตั้งค่าการเชื่อมต่อ Google Sheets (ไม่แก้)
# ==========================================
...
# ==========================================
# ✨ การตกแต่ง UI สไตล์ใหม่ สดใสเพื่อมือถือ (ชุดใหม่ค่ะเจ้านาย)
# ==========================================
...
# ==========================================
# ส่วนที่ 1: บันทึกรายการใหม่ (ไม่แก้)
# ==========================================
...
# ==========================================
# ส่วนที่ 2: Dashboard (ปรับเป็นกราฟโดนัท)
# ==========================================
...
if not df.empty:
    ...
    # สรุปตัวเลขแบบมินิมอล 3 ช่อง
    col1, col2, col3 = st.columns(3)
    col1.metric("รายรับ", f"{f_df['รายรับ'].sum():,.0f}")
    col2.metric("รายจ่าย", f"{f_df['รายจ่าย'].sum():,.0f}")
    col3.metric("คงเหลือ", f"{(f_df['รายรับ'].sum() - f_df['รายจ่าย'].sum()):,.0f}")

    # กราฟโดนัท
    exp_df = f_df[f_df['รายจ่าย'] > 0]
    if not exp_df.empty:
        cat_data = exp_df.groupby('รายการ', as_index=False)['รายจ่าย'].sum()
        # 💡 นี่คือจุดที่แก้ให้เป็นกราฟโดนัท พร้อมชื่อแนวนอนค่ะ!
        ...
        st.write("ไม่มีรายจ่ายเดือนนี้ค่ะ")
else:
